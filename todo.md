...
- [ ] DBスキーマにtask_categoriesとtask_definitionsテーブルを追加する
- [ ] pnpm db:pushでマイグレーションを実行する
- [ ] 現在のBASE_TASKSをDBに初期データとして投入する
- [ ] tRPCルーターにタスク定義のCRUD手続きを追加する
- [ ] 通常モードをDB参照に切り替える（BASE_TASKS配列を廃止）
- [ ] 編集モードUI（追加・インライン編集・削除・並び替え）を実装する
